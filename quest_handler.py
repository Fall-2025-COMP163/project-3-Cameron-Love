...
def accept_quest(character, quest_id, quest_data_dict):
    ...
    #Checks if quest exist
    if quest_id not in quest_data_dict:
        raise QuestNotFoundError(f"Quest '{quest_id}' does not exist.")
    
    quest = quest_data_dict[quest_id]

    #Checks if character meets the level requirement for the quest
    if character['level'] < quest['required_level']:
        raise InsufficientLevelError(f"Level {quest['required_level']} required to accept this quest.")
    
    #Checks if there is a prerequisite quest and if the character has completed it yet
...
def complete_quest(character, quest_id, quest_data_dict):
    ...
    #Removes the quest from active quest
    character['active_quests'].remove(quest_id)
    #Adds the quest to completed quest
    character['completed_quests'].append(quest_id)

    #Grants the proper rewards for the quest
...
def abandon_quest(character, quest_id):
    ...
    #Checks if the quest is actually active
    if quest_id not in character['active_quests']:
        raise QuestNotActiveError(f"Quest '{quest_id}' is not currently active.")
...
def get_active_quests(character, quest_data_dict):
    ...
    for quest_id in character['active_quests']:
        #Checks if the quest is still an availabe quest
...
def get_available_quests(character, quest_data_dict):
    ...
    #.item lets you look at the key and value at the same time
    for quest_id, quest in quest_data_dict.items():
        ...
        # Check if the character has already completed the quest
        if quest_id in character['completed_quests']:
            continue
        
        # Checks if the quest is already an active quest
        if quest_id in character['active_quests']:
            continue
        
        # If all the checks passed, this adds the quest to available quests
...
def is_quest_completed(character, quest_id):
    ...
    #Checks if the character completed the quest
...
def is_quest_active(character, quest_id):
    ...
    #Checks if the quest is an active quests
...
def can_accept_quest(character, quest_id, quest_data_dict):
    ...
    #Checks if character meets the level requirement
    if character['level'] < quest['required_level']:
        return False
    ...
    #Checks if the character already completed check
    if quest_id in character['completed_quests']:
        return False
    
    #Checks if the quest is already active
    if quest_id in character['active_quests']:
        return False
...
def get_quest_prerequisite_chain(quest_id, quest_data_dict):
    ...
    #Checks if the quest exist
    if quest_id not in quest_data_dict:
        raise QuestNotFoundError(f"Quest '{quest_id}' does not exist.")
    
    #Creats chain for prerequisite quest
    chain = []
    ...
    while True:
        ...
        #Adds the quest to the chain if it exist
    ...
    chain.reverse()  # Reverses the order to put the earliest prerequisite first
    return chain
...
def get_quest_completion_percentage(character, quest_data_dict):
    ...
    #Counts total quest in the game
    total_quests = len(quest_data_dict)

    #If there are no somehow no quest, returns 0.0 to avoid division by zero
    if total_quests == 0:
        return 0.0
    
    #Counts the number of completed quests by character
    completed_quests = len(character['completed_quests'])

    #Calcultes the percentage of quest completed by the user
...
def get_total_quest_rewards_earned(character, quest_data_dict):
    ...
    # Loops through each completed quest ID
    for quest_id in character["completed_quests"]:

        # Make sure the quest exists in the quest database
        if quest_id in quest_data_dict:
            ...
            # Adds the gold reward
            total_gold += quest["reward_gold"]

    # Returns results with a dictionary
    return {
        "total_xp": total_xp,
...
def get_quests_by_level(quest_data_dict, min_level, max_level):
    ...
    results = []  # list of quests that will be returned

    # Loops through every quest in the dictionary
    for quest_id, quest in quest_data_dict.items():

        # Extracts the required level for the quest
        level = quest["required_level"]

        # Checks if the characters level fits within the range
...
def display_character_quest_progress(character, quest_data_dict):
    ...
    # Counts the number of active quest for the character
    active_count = len(character['active_quests'])

    # Counts the number of quest the character has completed
    completed_count = len(character['completed_quests'])

    # Counts the percentage of the quest completed in the game
    completion_percentage = get_quest_completion_percentage(character, quest_data_dict)

    # Counts the amount of XP + gold earned from completed quests
    total_rewards = get_total_quest_rewards_earned(character, quest_data_dict)

    #Displays all of that information
    print("\n=== QUEST PROGRESS ===")
...
def validate_quest_prerequisites(quest_data_dict):
    ...
    for quest_id, quest in quest_data_dict.items():
        ...
        prereq = quest['prerequisite']  # Shows prerequisites for quest

        # If quest has no prerequisite then skip it
        if prereq == "NONE":
            continue
